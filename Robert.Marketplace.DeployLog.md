## 09.12.19 KAfka брокер на *.8.29 стал выжирать до 80% CPU на iowait.
После перезагрузки машины Zookeeper стал вести себя нестабильно, в мониторе DataDog появились записи о недоступности ноды. Брокер Kafka на порту 9091 также был недоступен. 
Из-за нагрузки CPU перестал работать Docker swarm node из-за чего сервисы, обслуживающиеся на ноде, перестали отвечать на запросы. Перезапуск контейнеров ни к чему не приводил

**Решение**: перезагрузил машину. После 30-50 мин KAfka кластер вернулся в зеленый статус. Docker node тоже вернулся в Up статус. 
**В случае отказа ноды надо переводить ее в статус DRAIN чтобы сервисы перекочевали на рабочие ноды, почему то Swarm автоматом этого не сделал, кроме того держать scaling фактор примерно равным кол-ву нод** 
